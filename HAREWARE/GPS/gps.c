...
//添加头文件: #include"gps.h"
//工程中添加usart2.c相关文件
//在主函数外添加GPS信息： nmea_msg gpsx;   												 												 
//主函数内初始化GPS：   GPS_Init();

//接收数据： 定义 float tp1,tp2,tp3,tp4  ；
//if(USART2_RX_STA&0X8000)		//接收到一次数据了
//		{
//			GPS_Analysis(&gpsx,(u8*)USART2_RX_BUF);//分析字符串
//			USART2_RX_STA=0;	
//      tp1=gpsx.longitude/100000; 获取经度
//      tp2=gpsx.latitude/100000;	 获取纬度
//      tp3=gpsx.altitude/10;      获取高度
//      tp4=gpsx.speed/1000;	     获取速度
//    }

//扩展部分：	 	   
//gpsx.posslnum		用于定位的卫星数
//gpsx.svnum%100 		可见卫星数
//gpsx.utc.year,gpsx.utc.month,gpsx.utc.date显示UTC日期(年/月/日)
//gpsx.utc.hour,gpsx.utc.min,gpsx.utc.sec);	显示UTC时间(时/分/秒)
