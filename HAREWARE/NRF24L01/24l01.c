...
//u8 tmp_buf[33]; 定义发送/接收包字符串
//NRF24L01_Init();   初始化无线模块


//检测NRF24L01是否在位，配合循环显示等待NRF24L01到位;   NRF24L01_Check() 返回0，成功；返回1，失败；
...
//发射端：
//NRF24L01_TX_Mode(); TX模式初始化

// for(t=0;t<32;t++) 
//		    {
//				 tmp_buf[t]=xxx;	通过循环语句给数据包中的每一位进行相应的赋值
//				}							 	  
//				tmp_buf[32]=0;  在字符串最后一位加入结束符	

// while(1)
//   	{	 

//	    if(NRF24L01_TxPacket(tmp_buf)==TX_OK)   检测到发送成功后跳出循环
//		    break；		
//    }   	 	  
				
		   	
//接收端：	
//NRF24L01_RX_Mode();	 RX模式初始化  
// while(1)       	
//		{	  		    		    				 
//			if(NRF24L01_RxPacket(tmp_buf)==0)  一旦接收到信息,跳出循环
//			{
